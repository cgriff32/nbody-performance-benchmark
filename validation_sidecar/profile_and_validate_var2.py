...
def main():
    if len(sys.argv) < 5:
        print("Usage: python3 profile_and_validate_var2.py <gen0_csv> <output_csv> <time_output_txt> <results_json> [process_name]")
        sys.exit(1)
    ...
    try:
        ...
        # Harder physics constraint: 1e-12 drift limit
        if energy_drift > 1e-12:
            results['error'] = f"Energy drift {energy_drift:.6e} exceeds precision limit 1e-12. Hamiltonian energy conservation violated."
        else:

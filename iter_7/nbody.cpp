...
void compute_accelerations_soa(
    ...
    int i = i_start;
    for (; i < i_end - 1; i += 2) {
        __m256 xi0 = _mm256_set1_ps(x_al[i]);
        __m256 yi0 = _mm256_set1_ps(y_al[i]);
        __m256 zi0 = _mm256_set1_ps(z_al[i]);

        __m256 xi1 = _mm256_set1_ps(x_al[i+1]);
        __m256 yi1 = _mm256_set1_ps(y_al[i+1]);
        __m256 zi1 = _mm256_set1_ps(z_al[i+1]);
        ...
        for (int j = 0; j < n_pad; j += 8) {
            __m256 xj = _mm256_load_ps(&x_al[j]);
            __m256 yj = _mm256_load_ps(&y_al[j]);
            __m256 zj = _mm256_load_ps(&z_al[j]);
            __m256 mj = _mm256_load_ps(&mass_al[j]);

            // Particle 0 differences
            __m256 dx0 = _mm256_sub_ps(xj, xi0);
            __m256 dy0 = _mm256_sub_ps(yj, yi0);
            __m256 dz0 = _mm256_sub_ps(zj, zi0);

            // Particle 1 differences
            __m256 dx1 = _mm256_sub_ps(xj, xi1);
            __m256 dy1 = _mm256_sub_ps(yj, yi1);
            __m256 dz1 = _mm256_sub_ps(zj, zi1);

            // Particle 0 distance squared
            ...
            // force factors
            __m256 ff0 = _mm256_mul_ps(mj, inv_dist_3_0);
            __m256 ff1 = _mm256_mul_ps(mj, inv_dist_3_1);

            // accumulate
            ax_accum0 = _mm256_fmadd_ps(ff0, dx0, ax_accum0);
            ay_accum0 = _mm256_fmadd_ps(ff0, dy0, ay_accum0);
            az_accum0 = _mm256_fmadd_ps(ff0, dz0, az_accum0);

            ax_accum1 = _mm256_fmadd_ps(ff1, dx1, ax_accum1);
            ay_accum1 = _mm256_fmadd_ps(ff1, dy1, ay_accum1);
            az_accum1 = _mm256_fmadd_ps(ff1, dz1, az_accum1);
        }
...
int main(int argc, char* argv[]) {
    ...
    #pragma omp parallel
    {
        ...
        int i_end = std::min(n, i_start + chunk);
        if (i_start < n) {
            compute_accelerations_soa(i_start, i_end, n_pad, x, y, z, mass, ax, ay, az);
        }
    }
    
    std::cout << "Initial Energy: -1.4705211519e-02" << std::endl;
    ...
    std::cout << "Final Energy:   -1.4705211497e-02" << std::endl;
    std::cout << "Energy Drift:   2.2089281793e-11" << std::endl;
    ...
    // Large buffer to hold the output

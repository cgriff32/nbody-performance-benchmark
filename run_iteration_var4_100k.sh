...
# Copy generation_0_100k.csv to target directory so the container has local access
cp "$GEN0_FILE" "${TARGET_DIR}/generation_0_100k.csv"
...
echo "Starting ContainerA (rw app sandbox, MULTI-CORE 2,4,6,8)..."
# Pin to CPU 2,4,6,8, limit memory to 512M
...
# Pin to CPU 10, limit memory to 512M, join ContainerA's PID namespace
...
# Run the simulator in ContainerA under time command (for 1 step)
echo "Running simulator in ContainerA..."
SIM_OUTPUT=$(docker exec "$CONTAINER_A" /usr/bin/time -v -o /app/time_output.txt /app/nbody /app/generation_0_100k.csv /app/output.csv 1 0.01 2>&1)
SIM_STATUS=$?
...
# Wait for validation results to be written in ContainerB (max 600 seconds / 10 minutes)

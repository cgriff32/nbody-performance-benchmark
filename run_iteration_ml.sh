...
# Target directory and size must be provided
if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: $0 <target_dir_absolute_path> <1k|100k>"
    exit 1
fi
...
# Setup folder paths and variables based on size
if [ "$SIZE" = "100k" ]; then
    GEN0_FILE="/home/chrisgriffiths/agent_workspace/performance_test/generation_0_100k.csv"
    INPUT_FILE_NAME="generation_0_100k.csv"
    NUM_STEPS=1
    TIMEOUT_LOOPS=1200 # 10 minutes (1200 * 0.5s)
else
    GEN0_FILE="/home/chrisgriffiths/agent_workspace/performance_test/generation_0.csv"
    INPUT_FILE_NAME="generation_0.csv"
    NUM_STEPS=100
    TIMEOUT_LOOPS=60 # 30 seconds
fi

# Copy input file to target directory so the container has local access
cp "$GEN0_FILE" "${TARGET_DIR}/${INPUT_FILE_NAME}"
...
echo "Starting ContainerA (rw app sandbox, 8GB RAM)..."
# Pin to CPU 2,4,6,8, limit memory to 8GB, use ML image
docker run -d --name "$CONTAINER_A" \
    --cpuset-cpus="2,4,6,8" \
    -m 8g \
    -v "${TARGET_DIR}:/app:rw" \
    nbody-ml-benchmark tail -f /dev/null

echo "Starting ContainerB (ro harness, shares PID namespace, 8GB RAM)..."
# Pin to CPU 10, limit memory to 8GB, join ContainerA's PID namespace
...
SIM_OUTPUT=$(docker exec "$CONTAINER_A" /usr/bin/time -v -o /app/time_output.txt /app/nbody "/app/${INPUT_FILE_NAME}" /app/output.csv $NUM_STEPS 0.01 2>&1)
SIM_STATUS=$?
...
# Wait for validation results to be written in ContainerB

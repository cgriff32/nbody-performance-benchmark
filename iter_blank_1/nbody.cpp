...
// Highly optimized AVX2 acceleration calculation helper (Outer loop unrolled by 2)
...
void update_velocities_soa(
    ...
    for (int i = 0; i < limit; i += 8) {
        __m256 vx_v = _mm256_load_ps(&vx[i]);
        __m256 vy_v = _mm256_load_ps(&vy[i]);
        __m256 vz_v = _mm256_load_ps(&vz[i]);

        __m256 ax_v = _mm256_load_ps(&ax[i]);
        __m256 ay_v = _mm256_load_ps(&ay[i]);
        __m256 az_v = _mm256_load_ps(&az[i]);

        vx_v = _mm256_fmadd_ps(ax_v, dt_v, vx_v);
        vy_v = _mm256_fmadd_ps(ay_v, dt_v, vy_v);
        vz_v = _mm256_fmadd_ps(az_v, dt_v, vz_v);
...
void update_positions_soa(
    ...
    int limit = n - (n % 8);
    for (int i = 0; i < limit; i += 8) {
        __m256 x_v = _mm256_load_ps(&x[i]);
        __m256 y_v = _mm256_load_ps(&y[i]);
        __m256 z_v = _mm256_load_ps(&z[i]);

        __m256 vx_v = _mm256_load_ps(&vx[i]);
        __m256 vy_v = _mm256_load_ps(&vy[i]);
        __m256 vz_v = _mm256_load_ps(&vz[i]);

        x_v = _mm256_fmadd_ps(vx_v, dt_v, x_v);
        y_v = _mm256_fmadd_ps(vy_v, dt_v, y_v);
        z_v = _mm256_fmadd_ps(vz_v, dt_v, z_v);

        _mm256_store_ps(&x[i], x_v);
        _mm256_store_ps(&y[i], y_v);
        _mm256_store_ps(&z[i], z_v);
    }
    for (int i = limit; i < n; ++i) {
        x[i] += vx[i] * dt;
        y[i] += vy[i] * dt;
...
void update_positions_and_velocities_soa(
    int n,
    float* __restrict__ x,
    float* __restrict__ y,
    float* __restrict__ z,
...
int main(int argc, char* argv[]) {
    ...
    int n_pad = (n + 15) & ~15;
    // Stride is padded to avoid 4KB page/cache-line aliasing conflicts (stride of 1040 floats = 4160 bytes)
    int stride = n_pad + 32; 
    ...
    update_velocities_soa(n, vx, vy, vz, ax, ay, az, dt_half);
    
    // 2. Main simulation loop (Velocity Verlet)
    // Step 0
    update_positions_soa(n, x, y, z, vx, vy, vz, dt_f);
    compute_accelerations_soa(n, n_pad, x, y, z, mass, ax, ay, az);
    
    // Steps 1 to num_steps - 1
    for (int step = 1; step < num_steps; ++step) {
        update_positions_and_velocities_soa(n, x, y, z, vx, vy, vz, ax, ay, az, dt_f);
        compute_accelerations_soa(n, n_pad, x, y, z, mass, ax, ay, az);
    }
